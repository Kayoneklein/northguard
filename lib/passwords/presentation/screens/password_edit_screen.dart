...
class PasswordEditScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider<pass_edit.PasswordEditBloc>(
      create: (_) => pass_edit.PasswordEditBloc(
        password: password,
...
class _PasswordEditFormState extends State<PasswordEditForm> {
  late final pass_edit.PasswordEditBloc _bloc;
  ...
  void initState() {
    super.initState();
    _bloc = BlocProvider.of<pass_edit.PasswordEditBloc>(context);
  ...
  void _initControllers(pass_edit.PasswordEditState state) {
  ...
  @override
  Widget build(BuildContext context) {
    return BlocListener<auth.AuthenticationBloc, auth.AuthenticationState>(
      listener: (context, state) async {
        if (state is auth.Unauthenticated) {
          Navigator.of(context).pop();
        }
      },
      child: BlocListener<pass_edit.PasswordEditBloc, pass_edit.PasswordEditState>(
        listener: _blocCommandListener,
        child: BlocBuilder<pass_edit.PasswordEditBloc, pass_edit.PasswordEditState>(
  ...
  Widget _buildEndDrawer(
    ...
    pass_edit.PasswordEditState state,
  ) {
    switch (state.drawerPage) {
      case pass_edit.DrawerPage.selectTags:
        return TagsEndDrawer(state: state, scaffoldKey: _scaffoldKey);
      case pass_edit.DrawerPage.addLocation:
      ...
      case pass_edit.DrawerPage.editLocation:
        return LocationEndDrawer(
          state: state,
          isNew: false,
          titleController: _locationTitleController,
  ...
  void _onNameChanged() {
    _bloc.add(pass_edit.NameChanged(name: _nameController.text));
  }

  void _onUsernameChanged() {
    _bloc.add(pass_edit.UsernameChanged(username: _usernameController.text));
  }

  void _onPasswordChanged() {
    _bloc.add(pass_edit.PasswordChanged(password: _passwordController.text));
  }

  void _onUrlChanged() {
    _bloc.add(pass_edit.UrlChanged(url: _urlController.text));
  }

  void _onNotesChanged() {
    _bloc.add(pass_edit.NotesChanged(notes: _notesController.text));
  }

  void _onLocationTitleChanged() {
    _bloc.add(
      pass_edit.LocationTitleChanged(title: _locationTitleController.text),
    );
  }

  void _onLocationLatitudeChanged() {
    _bloc.add(
      pass_edit.LocationLatitudeChanged(
  ...
  void _onLocationLongitudeChanged() {
    _bloc.add(
      pass_edit.LocationLongitudeChanged(
  ...
  void _onLocationAccuracyChanged() {
    _bloc.add(
      pass_edit.LocationAccuracyChanged(
  ...
  void _onLocationAddressChanged() {
    _bloc.add(
      pass_edit.LocationAddressChanged(
  ...
  void _onCancelPressed() {
    ...
    _bloc.add(pass_edit.FormCanceled());
  }
  ...
  Future<void> _uploadFile(BuildContext context) async {
    ...
    if (result != null) {
      _bloc.add(
        pass_edit.FileForUploadPicked(path: result.files.single.path ?? ''),
      );
    }
  }
  ...
  Future<void> _blocCommandListener(
    ...
    if (_scaffoldKey.currentState?.isEndDrawerOpen == true &&
        state.drawerPage == pass_edit.DrawerPage.none) {
      Navigator.of(context).pop();
    }
    if (state is pass_edit.TagCreationPermittedState) {
      _createNewTag(context);
    }
    if (state is pass_edit.FileUploadPermittedState) {
      _uploadFile(context);
    }
    if (state is pass_edit.FileUploadResultState) {
      String text;
      switch (state.uploadResult) {
        case pass_edit.FileUploadResult.success:
          text = Strings.uploadFileResultSuccess;
          break;
        case pass_edit.FileUploadResult.storageFull:
          text = Strings.uploadFileResultStorageFull;
          break;
        case pass_edit.FileUploadResult.largerThan1Mb:
        case pass_edit.FileUploadResult.otherError:
    ...
    if (state is pass_edit.FinishEditState) {
      Navigator.of(context).pop(state.result);
    }
    if (state is pass_edit.ConfirmDiscardChangesState) {
      final bool result =
          ...
          false;
      _bloc.add(pass_edit.ConfirmationSubmitted(isConfirmed: result));
    }
    if (state is pass_edit.PremiumRequiredState) {
      showPremiumRestrictionsDialog(context);
    }
    if (state is pass_edit.SessionExpiredState) {
      BlocProvider.of<auth.AuthenticationBloc>(
        context,
      ).add(auth.SessionExpiredEvent());
    }
    if (state is pass_edit.ConnectionErrorState) {
  ...
  Future<void> _createNewTag(BuildContext context) async {
    ...
    if (name != null) {
      _bloc.add(pass_edit.NewTagAdded(name: name));
    }
  }
}
