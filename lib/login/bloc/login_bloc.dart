...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  Future<void> _loadLoginData() async {
    final isFromAutofill = await Preferences().isFromAutofill;
    final _firstTimeLoginServer = await Preferences().firstTimeLoginServer;
    final latestEmail = await Preferences().latestEmail;
    final _autoLogout = await Preferences().autoLogout;

    final configResults = await WebProvider.get.getRemoteConfig();
    final RemoteConfiguration remoteConfiguration =
        configResults.data ?? RemoteConfiguration.initial;

    if (isFromAutofill != true) {
      if (remoteConfiguration.disableConfigServer == false) {
        if (_firstTimeLoginServer != true) {
          add(VeryFirstTimeLogin());
        }
      }
    }

    final latestLogin = latestEmail ?? '';

    if (latestLogin != state.email) {
      add(EmailChanged(email: latestLogin));
    }

    final autoLogout = AutoLogoutTimer.values[int.parse(_autoLogout ?? '0')];
    ...
    final biometricsChecked = await Preferences().biometricCheckbox;
