...
class SignUpScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context) => signup.SignUpBloc(),
        child: SignUpForm(),
      ),
...
class _SignUpFormState extends State<SignUpForm> {
  late final signup.SignUpBloc _bloc;
  ...
  void initState() {
    super.initState();
    _bloc = BlocProvider.of<signup.SignUpBloc>(context);
  ...
  Widget build(BuildContext context) {
    return BlocListener<signup.SignUpBloc, signup.SignUpState>(
      listener: (context, state) {
        // Success
        if (state.signUpStatus == signup.SignUpStatus.success) {
          BlocProvider.of<AuthenticationBloc>(
            context,
          ).add(const SignedInEvent());
        }
        // Errors
        if (state.signUpStatus == signup.SignUpStatus.invalidEmail) {
          _showErrorDialog(Strings.signUpErrorInvalidEmail);
        } else if (state.signUpStatus == signup.SignUpStatus.accountExists) {
          _showErrorDialog(Strings.signUpErrorAccountExists);
        } else if (state.signUpStatus == signup.SignUpStatus.otherError) {
