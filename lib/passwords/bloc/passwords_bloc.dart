...
import 'package:northguard/authentication/bloc/authentication.dart'
    as auth_bloc;
import 'package:northguard/authentication/bloc/authentication_state.dart'
    as auth_state;
...
import 'package:northguard/model/sorting_order.dart';
  ...
  PasswordsBloc({
    ...
    on<PasswordsEvent>((event, emit) async {
      ...
      if (event is PasswordAddPressed) {
        ...
        final i = await _loadPasswordIcon(event.password.url);
        event.password.icon = i;
      ...
      if (event is PasswordUpdated) {
        ...
        if (state.myPasswords.any((p) => p.tempId == event.password.tempId)) {
          emit(state.copyWith(isLoading: true));
          final passwordIcon = await _loadPasswordIcon(event.password.url);
          event.password.icon = passwordIcon;
          final BuiltList<Password> passwords = state.myPasswords.rebuild(
            (b) => b.map(
