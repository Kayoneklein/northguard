...
class _PasswordsScreenState extends State<PasswordsScreen> {
  ...
  Future<void> _viewPasswordDetails(
    ...
    if (result != null) {
      BlocProvider.of<pass.PasswordsBloc>(
        context,
      ).add(pass.PasswordUpdated(password: result));
  ...
  Future<void> _sharePassword(BuildContext context, Password password) async {
    ...
    BlocProvider.of<pass.PasswordsBloc>(
      context,
    ).add(pass.PasswordSharePressed(password: password));
  }
          ...
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                BlocProvider.of<pass.PasswordsBloc>(
                  context,
                ).add(pass.PasswordDeletePressed(password: password));
              },
            ),
          ],
        );
  ...
  void _showShareInfo(BuildContext context, Password password) {
    BlocProvider.of<pass.PasswordsBloc>(
      context,
    ).add(pass.PasswordShareInfoPressed(password: password));
  ...
  Widget build(BuildContext context) {
    return BlocListener<pass.PasswordsBloc, pass.PasswordsState>(
      listener: (context, state) async {
        ...
        // }
        if (state is pass.ConnectionErrorState) {
          // show error dialog
          connectionError(context, state.errorMessage);
        }
        if (state is pass.PremiumRequiredState) {
          showPremiumRestrictionsDialog(context);
        }
        if (state is pass.PasswordsSavedState) {
          _passwordsUpdateResult(context, state.isSavedSuccessfully);
        }
        if (state is pass.PasswordsSharedState) {
          _passwordsShareResult(context, state.isSharedSuccessfully);
        }
        if (state is pass.LocationServiceErrorState) {
          _locationServiceError(context);
        }
        if (state is pass.SharingPermittedState) {
          final result = await Navigator.push(
            context,
          ...
          if (result != null) {
            BlocProvider.of<pass.PasswordsBloc>(context).add(
              pass.SharingMembersSelected(
        ...
        if (state is pass.ShowSavePasswordToastState) {
          Fluttertoast.showToast(msg: Strings.messagePasswordSavedSuccess);
          SystemNavigator.pop();
        }

        if (state is pass.ShowReadOnlyModeToastState) {
          Fluttertoast.showToast(msg: Strings.readOnlyToastAlert);
        }
        if (state is pass.SaveOrUpdateConfirmationRequestedState) {
          // show dialog for the user to choose if he wants to
          // save new password or update the existing password
          final needToShowDialog = !BlocProvider.of<pass.PasswordsBloc>(
            context,
  ...
  Widget _buildList(BuildContext context, pass.PasswordsState state) {
      ...
      child: PasswordList(
        ...
        onItemClicked: (position, password) async {
          if (await _settings.getBoolean(Settings.IS_FROM_AUTO_FILL_REQUEST)) {
            BlocProvider.of<pass.PasswordsBloc>(
              context,
            ).add(pass.AutofillPasswordEvent(password: password));
          } else if (BlocProvider.of<pass.PasswordsBloc>(
            context,
          ).isLoadingPasswordFromAutofill) {
            ...
            BlocProvider.of<pass.PasswordsBloc>(
              context,
            ).add(pass.UpdatePasswordFromAutofill(passwordToUpdate: password));
          } else {
            if (state.selectionModeActive) {
              BlocProvider.of<pass.PasswordsBloc>(
                context,
              ).add(pass.PasswordPressed(password: password));
            } else {
        ...
        onItemLongClicked: (position, password) {
          if (!state.isFromAutoFillRequestedApp) {
            BlocProvider.of<pass.PasswordsBloc>(
              context,
            ).add(pass.PasswordLongPressed(password: password));
          }
        },
  ...
  void _showDialogSaveOrUpdateConfirmationDialog(
    BuildContext context,
    pass.SaveOrUpdateConfirmationRequestedState state,
  ) {
    if (Platform.isAndroid) {
      showDialog<void>(
        context: context,
  ...
  void _onTappedSaveNew(
    ...
    BlocProvider.of<pass.PasswordsBloc>(context).add(
      pass.SaveNewPasswordFromAutofill(
  ...
  void _onTappedUpdateExisting(BuildContext context) {
    ...
    BlocProvider.of<pass.PasswordsBloc>(
      context,
    ).add(const pass.LoadInitialPasswordsForUpdateFromAutofill());
    Navigator.pop(context);
  }
}
